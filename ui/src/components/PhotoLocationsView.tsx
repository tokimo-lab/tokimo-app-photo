...
import type { PhotoOutput } from "../lib/types";
...
export function PhotoLocationsView({
  ...
  const statsQuery = api.photo.getLocationStats.useQuery(
    { id: appId! },
    { enabled: !!appId },
  );

  const photosQuery = api.photo.getPhotosByLocation.useQuery(
    {
      id: appId!,
  ...
  const groups = useMemo<LocationGroup[]>(() => {
    ...
    if (drill.level === "city") {
      const map = new Map<string, number>();
      for (const g of data) {
        if (g.geoProvince !== drill.province) continue;
        const key = g.geoCity || "未知";
        map.set(key, (map.get(key) || 0) + g.count);
      }
      return Array.from(map.entries())
        .map(([city, count]) => ({
          geoProvince: drill.province ?? null,
          geoCity: city,
          geoDistrict: null,
          count,
        }))
    ...
    if (drill.level === "district") {
      return data
        .filter(
          (g) =>
            g.geoProvince === drill.province && g.geoCity === drill.city,
        )
        .sort((a, b) => b.count - a.count);
    }
  ...
  const handleDrill = useCallback(
    (group: LocationGroup) => {
      if (drill.level === "province") {
        setDrill({
          level: "city",
          province: group.geoProvince ?? undefined,
        });
      } else if (drill.level === "city") {
        setDrill({
          level: "district",
          province: drill.province,
          city: group.geoCity ?? undefined,
        });
        ...
        setDrill({
          ...
          province: drill.province,
          city: drill.city,
          district: group.geoDistrict ?? undefined,
        });
      }
    },
    [drill],
        ...
        <div className="grid grid-cols-2 gap-2 px-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          {groups.map((g) => {
            ...
            return (
              <button
                type="button"
                key={`${g.geoProvince}-${g.geoCity}-${g.geoDistrict}`}
