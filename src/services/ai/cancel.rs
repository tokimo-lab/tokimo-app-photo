...
tokio::task_local! {
    ...
    /// handler. Reads via [`current_job`]; unset outside queue handlers.
    static CURRENT_JOB: CurrentJobContext;
}
...
/// Snapshot of the current job context if the caller is running inside a
/// queue handler; `None` otherwise. Cheap (clones small handles).
pub fn current_job() -> Option<CurrentJobContext> {
    CURRENT_JOB.try_with(Clone::clone).ok()
}

/// Run `fut` with the given [`CurrentJobContext`] installed as the
/// task-local. Used by the queue worker around `handlers::dispatch`.
