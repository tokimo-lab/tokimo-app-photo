...
fn build_router(ctx: Arc<AppCtx>) -> Router {
    Router::new()
        // PhotoLibrary CRUD
        .route("/", get(handlers::list_photo_libraries).post(handlers::create_photo_library))
        ...
        .route("/{id}/photo-albums", get(handlers::list_photo_albums).post(handlers::create_album))
        // Albums (album-scoped)
        .route("/albums/{id}", delete(handlers::delete_album))
        .route("/albums/{id}/photos", get(handlers::list_album_photos))
        .route("/albums/{id}/add-photos", post(handlers::add_photos_to_album))
        .route("/albums/{id}/remove-photos", post(handlers::remove_photos_from_album))
        // Individual photo
        .route("/item/{id}", get(handlers::get_photo).patch(handlers::update_photo))
        .route("/item/{id}/toggle-favorite", post(handlers::toggle_favorite))
        ...
        .route("/item/{id}/faces/{face_id}/assign", patch(handlers::assign_face_to_person))
        .route("/item/{id}/faces/{face_id}/create-person", post(handlers::create_person_from_face))
        ...
        .route("/item/{id}/refresh-thumbnail", post(handlers::refresh_thumbnail))
        .route("/item/{id}/ocr-results", get(handlers::get_photo_ocr_results).post(handlers::create_ocr_result))
        // Batch (library-scoped)
        .route("/{id}/photos/batch-favorite", post(handlers::batch_favorite))
        .route("/{id}/photos/batch-delete", post(handlers::batch_delete))
        .route("/{id}/photos/batch-hide", post(handlers::batch_hide))
        .route("/{id}/photos/trash", get(handlers::list_trashed).post(handlers::trash_photos))
        .route("/{id}/photos/restore", post(handlers::restore_photos))
        .route("/{id}/photos/permanent-delete", post(handlers::permanent_delete))
        .route("/{id}/photos/rescan", post(handlers::rescan))
        // Geo
        .route("/{id}/photos/reverse-geocode", post(handlers::reverse_geocode))
        .route("/{id}/photos/map-points", get(handlers::map_points))
        .route("/{id}/photos/locations", get(handlers::location_stats))
        .route("/{id}/photos/by-location", get(handlers::photos_by_location))
        ...
        .route("/{id}/photos/ocr-results", delete(handlers::clear_ocr_results))
        .route("/{id}/photos/face-results", delete(handlers::clear_face_results))
        .route("/{id}/photos/clip-results", delete(handlers::clear_clip_results))
        .route("/{id}/photos/thumbnails", delete(handlers::clear_thumbnails))
        ...
        .route("/{id}/persons/{person_id}/photos", get(handlers::person_photos))
        .route("/{id}/persons/{person_id}", patch(handlers::rename_person))
        // OCR CRUD
        .route("/ocr-results/{ocr_id}", patch(handlers::update_ocr_result).delete(handlers::delete_ocr_result))
        // Settings
        .route("/settings/geo", get(handlers::get_photo_geo_settings).put(handlers::update_photo_geo_settings))
        .route("/settings/geo/test", post(handlers::test_photo_geo_connection))
        .route("/settings/ai", get(handlers::get_photo_ai_settings).put(handlers::update_photo_ai_settings))
        .route("/settings/ai/test", post(handlers::test_photo_ai_connection))
        .route("/settings/ai/ocr-results", delete(handlers::clear_all_ocr_results))
        // Library detail (after named routes)
        .route("/{id}", get(handlers::get_photo_library).patch(handlers::update_photo_library).delete(handlers::delete_photo_library))
