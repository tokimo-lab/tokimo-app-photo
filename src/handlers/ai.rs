...
pub async fn refresh_clip(
    ...
    let client = state.bus_client.get()
        .ok_or_else(|| AppError::Internal("BusClient not yet bound".into()))?;
    let job = jobs::enqueue_with_dedupe(
        client,
        client.auto_caller("photo"),
        jobs::CreateJobRequest {
            job_type: "photo_clip_single".to_string(),
            params: serde_json::json!({ "photoId": photo_id.to_string() }),
            data: None,
            parent_job_id: None,
            task_type: None,
            dedupe_key: Some(photo_id.to_string()),
            priority: Some(crate::queue::JobPriority::UserAction.as_i32()),
        },
    )
    .await?;
...
pub async fn refresh_faces(
    ...
    let job = jobs::enqueue_with_dedupe(
        ...
        client.auto_caller("photo"),
        jobs::CreateJobRequest {
            job_type: "photo_face_single".to_string(),
            params: serde_json::json!({ "photoId": photo_id.to_string() }),
            data: None,
            parent_job_id: None,
            task_type: None,
...
pub async fn refresh_ocr(
    ...
    let job = jobs::enqueue_with_dedupe(
        ...
        client.auto_caller("photo"),
        jobs::CreateJobRequest {
            job_type: "photo_ocr_single".to_string(),
            params: serde_json::json!({ "photoId": photo_id.to_string() }),
            data: None,
            parent_job_id: None,
            task_type: None,
