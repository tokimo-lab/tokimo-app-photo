...
pub async fn reverse_geocode(
    ...
    use crate::apps::photo::services::photo_geo::PhotoGeoService;
...
pub async fn photos_by_location(
    ...
    let photos =
        PhotoRepo::list_by_location(&state.db, app_id, &page, q.province.as_deref(), q.city.as_deref(), q.district.as_deref())
            .await?;
...
pub async fn test_photo_geo_connection(
    ...
    use crate::config::PhotoGeoSettings;
    use crate::db::repos::system_config_repo::SystemConfigRepo;
    use crate::apps::photo::services::photo_geo::reverse_geocode_dispatch;
    ...
    match settings.provider.as_str() {
        ...
        "tianditu" => {
            if let Some(bk) =
                settings.tianditu_browser_key.as_deref().filter(|k| !k.is_empty())
            {
...
async fn test_tianditu_browser_key(
    http: &reqwest::Client,
    tk: &str,
) -> serde_json::Value {
    let url = format!(
