...
/**
 * Build thumbnail URL for a photo via the unified /api/thumb endpoint.
 * Backend resolves VFS path from DB, generates via libvips, caches in storage.
 */
export function photoThumbUrl(photo: { id: string }): string {
  return thumbUrl("photo", photo.id, THUMB_WIDTH);
}
